<script>
    // Function to format child row for Koreksi (product details)
    function formatChildRowKoreksi(d) {
...
<script>
    // Initialize DataTable for Koreksi (DB - Grouped by Invoice)
    function initializeDataTableKoreksiDb() {
        if ($.fn.DataTable.isDataTable('#table-koreksi')) {
            $('#table-koreksi').DataTable().destroy();
        }

        tableKoreksiDb = $('#table-koreksi').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-koreksi" data-invoice="${row.no_invoice}" ${checked}>`;
                    }
                },
                {
                    data: 'customer_id',
            ...
            initComplete: function() {
                ...
                api.columns().every(function() {
                    var column = this;
                    var input = $('.column-filter-koreksi[data-column="' + (column.index()) + '"]');
        ...
        $('#table-koreksi tbody').on('click', 'td.dt-control', function() {
            var tr = $(this).closest('tr');
            var row = tableKoreksiDb.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                row.child(formatChildRowKoreksi(row.data())).show();
                tr.addClass('shown');

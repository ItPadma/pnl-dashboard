<script>
    // Function to format child row for Pending (product details)
    function formatChildRowPending(d) {
...
<script>
    // Initialize DataTable for Pending (DB - Grouped by Invoice)
    function initializeDataTablePendingDb() {
        if ($.fn.DataTable.isDataTable('#table-pending')) {
            $('#table-pending').DataTable().destroy();
        }

        tablePendingDb = $('#table-pending').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-pending" data-invoice="${row.no_invoice}" ${checked}>`;
                    }
                },
                {
                    data: 'customer_id',
            ...
            initComplete: function() {
                ...
                api.columns().every(function() {
                    var column = this;
                    var input = $('.column-filter-pending[data-column="' + (column.index()) + '"]');
        ...
        $('#table-pending tbody').on('click', 'td.dt-control', function() {
            var tr = $(this).closest('tr');
            var row = tablePendingDb.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                row.child(formatChildRowPending(row.data())).show();
                tr.addClass('shown');

<script>
    // Initialize new DataTable for Koreksi
    function initializeDataTableKoreksi() {
        if ($.fn.DataTable.isDataTable('#table-koreksi')) {
            $('#table-koreksi').DataTable().destroy();
        }

        tableKoreksi = $('#table-koreksi').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-koreksi" data-id="${row.id}" ${checked}>`;
                    }
                },
                {
                    data: 'id',
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<select id="move-to-${row.id}" class="form-select move-to" data-id="${row.id}" data-from="koreksi" ${checked}>

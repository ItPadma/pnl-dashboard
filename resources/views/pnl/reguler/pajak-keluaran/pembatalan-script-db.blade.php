<script>
    // Function to format child row for Pembatalan (product details)
    function formatChildRowPembatalan(d) {
...
<script>
    // Initialize DataTable for Pembatalan (DB - Grouped by Invoice)
    function initializeDataTablePembatalanDb() {
        if ($.fn.DataTable.isDataTable('#table-pembatalan')) {
            $('#table-pembatalan').DataTable().destroy();
        }

        tablePembatalanDb = $('#table-pembatalan').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-pembatalan" data-invoice="${row.no_invoice}" ${checked}>`;
                    }
                },
                {
                    data: 'customer_id',
        ...
        $('#table-pembatalan tbody').on('click', 'td.dt-control', function() {
            var tr = $(this).closest('tr');
            var row = tablePembatalanDb.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                row.child(formatChildRowPembatalan(row.data())).show();
                tr.addClass('shown');

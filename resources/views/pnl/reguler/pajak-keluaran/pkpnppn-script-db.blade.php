<script>
    // Function to format child row for PKP Non-PPN (product details)
    function formatChildRowPkpNppn(d) {
...
<script>
    // Initialize DataTable for PKP Non-PPN (DB - Grouped by Invoice)
    function initializeDataTablePkpNppnDb() {
        if ($.fn.DataTable.isDataTable('#table-pkpnppn')) {
            $('#table-pkpnppn').DataTable().destroy();
        }

        tablePkpDbNppn = $('#table-pkpnppn').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-pkpnppn" data-invoice="${row.no_invoice}" ${checked}>`;
                    }
                },
                {
                    data: 'customer_id',
        ...
        $('#table-pkpnppn tbody').on('click', 'td.dt-control', function() {
            var tr = $(this).closest('tr');
            var row = tablePkpDbNppn.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                row.child(formatChildRowPkpNppn(row.data())).show();
                tr.addClass('shown');

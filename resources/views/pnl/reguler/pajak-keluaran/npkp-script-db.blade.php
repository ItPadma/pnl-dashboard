<script>
    // Function to format child row for Non-PKP (product details)
    function formatChildRowNpkp(d) {
        ...
        html += '<div style="font-size:0.68rem;color:#6b7280;margin-bottom:2px;font-weight:600;">' + d.products.length +
            ' produk</div>';
        if (d.is_netted) {
            html += '<div style="font-size:0.68rem;color:#198754;margin-bottom:2px;font-weight:600;">' +
                '<i class="fas fa-check-circle"></i> Invoice sudah di-nett. Nilai Nett: ' +
                parseFloat(d.nett_dpp_ppn || 0).toLocaleString('id-ID', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }) +
                '</div>';
...
<script>
    // Initialize DataTable for Non-PKP (DB - Grouped by Invoice)
    function initializeDataTableNonPkpDb() {
        if ($.fn.DataTable.isDataTable('#table-npkp')) {
            $('#table-npkp').DataTable().destroy();
        }

        tableNonPkpDb = $('#table-npkp').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-npkp" data-invoice="${row.no_invoice}" ${checked}>`;
                    }
                },
                {
                    data: 'customer_id',
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        var formatted = parseFloat(data || 0).toFixed(2);
                        if (row.is_netted) {
        ...
        $('#table-npkp tbody').on('click', 'td.dt-control', function() {
            var tr = $(this).closest('tr');
            var row = tableNonPkpDb.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                row.child(formatChildRowNpkp(row.data())).show();
                tr.addClass('shown');

<script>
    // Function to format child row for Non-PKP Non-PPN (product details)
    function formatChildRowNpkpNppn(d) {
...
<script>
    // Initialize DataTable for Non-PKP Non-PPN (DB - Grouped by Invoice)
    function initializeDataTableNonPkpNppnDb() {
        if ($.fn.DataTable.isDataTable('#table-npkpnppn')) {
            $('#table-npkpnppn').DataTable().destroy();
        }

        tableNonPkpDbNppn = $('#table-npkpnppn').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ $dtDataRoute }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<input type="checkbox" class="row-checkbox-npkpnppn" data-invoice="${row.no_invoice}" ${checked}>`;
                    }
                },
                {
                    data: 'customer_id',
        ...
        $('#table-npkpnppn tbody').on('click', 'td.dt-control', function() {
            var tr = $(this).closest('tr');
            var row = tableNonPkpDbNppn.row(tr);
            ...
            if (row.child.isShown()) {
                ...
                row.child(formatChildRowNpkpNppn(row.data())).show();
                tr.addClass('shown');

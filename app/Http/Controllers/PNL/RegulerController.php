...
class RegulerController extends Controller
{
    ...
    public function dtPKGetData(Request $request)
    {
        ...
        try {
            ...
            $columnName =
                $request->get('columns')[$columnIndex]['data'] ?? 'tgl_faktur_pajak';
    ...
    public function updateMove2(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'move_to' => 'required|in:pkp,pkpnppn,npkp,npkpnppn,retur,nonstandar|different:move_from',
        ]);
        ...
        try {
            ...
            DB::transaction(function () use ($request, $ids, $moveFrom, $moveTo): void {
    ...
    public function updateChecked(Request $request)
    {
        ...
        try {
            ...
            if (array_sum(array_map(fn ($mode) => $mode ? 1 : 0, $targetModes)) !== 1) {
                return response()->json([
                    'status' => false,
            ...
            elseif (! empty($validated['invoices'] ?? [])) {
                ...
                $matchedInvoices = (clone $scopedQuery)
                    ...
                    ->values()
                    ->all();
                $missingInvoices = array_values(array_diff($invoices, $matchedInvoices));
    ...
    private function buildNonStandarReason(
        ...
        if (in_array($nikLength, [15, 16], true) && $nikLastTwoDigits !== '00') {
            return 'NIK 15/16 digit dan 2 digit akhir bukan 00';
        }
    ...
    {
        $query->whereRaw(
            "(has_moved = 'y' AND moved_to = 'nonstandar') OR (has_moved = 'n' AND {$this->normalizedNikLengthSql()} IN (15, 16) AND {$this->normalizedNikLastTwoSql()} != '00') OR ".
    ...
    private function applyNonStandarReasonFilter($dbquery, string $keyword): void
    {
        ...
        $dbquery->where(function ($query) use ($normalizedKeyword) {
            ...
            if (
                ...
                str_contains($normalizedKeyword, '15') ||
                str_contains($normalizedKeyword, '16') ||
                str_contains($normalizedKeyword, '00')
            ) {
                $query->orWhereRaw(
                    "(has_moved = 'n' AND {$this->normalizedNikLengthSql()} IN (15, 16) AND {$this->normalizedNikLastTwoSql()} != '00')",
                );
                $applied = true;
            }
    ...
    private function isNonStandarFallback(
        ...
        return ! ($isPkpPpn || $isPkpNonPpn || $isNonPkp || $isNonPkpNonPpn || $isRetur);
    }
    ...
    private function resolveSortableColumn(?string $columnName): string
    {
        ...
        if (! is_string($columnName) || ! in_array($columnName, $allowedColumns, true)) {
